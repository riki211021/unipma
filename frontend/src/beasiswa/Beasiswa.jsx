...
export default function Beasiswa() {
  const beasiswaList = [
    { nama: "Beasiswa KIP", gambar: influencerImg },
    { nama: "Beasiswa Mahasiswa Berprestasi", gambar: keuskupanImg },
    { nama: "Beasiswa Mahasiswa Unggulan", gambar: prestasiImg },
    { nama: "Beasiswa Anak Anggota PGRI", gambar: keluargaImg },
    { nama: "Beasiswa Potongan Sekolah Mitra", gambar: guruImg },
    { nama: "Beasiswa Khusus Mahasiswa NTT", gambar: beasiswa7Img },
    { nama: "Beasiswa Aktivis Paroki/Lingkungan/Stasi", gambar: beasiswa7Img },
  ];
  ...
  return (
    <div className="relative py-20 bg-gradient-to-br from-cyan-100 via-cyan-200 to-blue-200 overflow-hidden ui-text">
      ...
      <div className="relative text-center mb-16 space-y-4">
        ...
        <p className="text-gray-600 max-w-2xl mx-auto font-light">Berbagai pilihan beasiswa untuk mendukung perjalanan pendidikan Anda di UNIPMA</p>
      ...
      {/* Grid Beasiswa dengan Style Gambar + Teks Terintegrasi */}
      <div className="relative grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12 max-w-7xl mx-auto mb-16">
        {beasiswaList.map((item, index) => {
          ...
          const firstWord = words[0]; // "Beasiswa"
          const restWords = words.slice(1).join(" "); // Sisanya

          return (
            <div key={index} className="group relative">
              {/* Container untuk gambar dan teks */}
              <div className="relative">
                {/* Decorative dots di atas */}
                ...
                {/* Gambar */}
                <div className="relative z-10 transform translate-x-20 -translate-y 20 group-hover:translate-x-14 group-hover:-translate-y-12 transition-transform duration-500">
                  <img src={item.gambar} alt={item.nama} className="w-4/5 h-80 object-cover object-top rounded-2xl shadow-xl mx-auto" />
                </div>

                {/* Teks di bawah gambar, terintegrasi dengan layout */}
                <div className="relative -mt-8 z-20 px-4">
                  <div className="backdrop-blur-sm bg-white/95 rounded-xl shadow-xl p-5 border-l-4 border-cyan-500 group-hover:border-blue-600 transition-colors duration-300">
                    <h2 className="text-xl font-bold leading-tight">
                      <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-600 to-blue-600">{firstWord}</span>
                      <br />
      ...
      <div className="relative max-w-7xl mx-auto mt-12 grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="backdrop-blur-sm bg-white/60 border border-gray-100 rounded-xl p-6 text-center hover:shadow-lg transition-shadow">
          ...
          <p className="text-sm text-gray-600 font-light">Pilihan beasiswa yang beragam</p>
        </div>
        ...
        <div className="backdrop-blur-sm bg-white/60 border border-gray-100 rounded-xl p-6 text-center hover:shadow-lg transition-shadow">
          ...
          <p className="text-sm text-gray-600 font-light">Pendaftaran yang simpel dan cepat</p>
        </div>
      </div>
    </div>
  );
